...
export const CASES: CaseDef[] = [
  ...
  { key: 'autoinspect', idx: '06', projectIdx: 7, demoSrcs: ['./autoinspect_demo.mp4'], systemPrefixes: ['autoinspect'], tracks: ['vision', 'fullstack'] },
...
const en: Content = {
  ...
  systems: [
    ...
    },
    {
      ...
      badge: 'Computer Vision',
      badgeType: 'vision',
      title: 'Two Models, Intersected — AutoInspect AI',
      desc: 'A photo walk-around becomes a priced repair. Two YOLO11-seg models — 23 body panels and 8 defect types — are intersected mask to mask, so a finding reads "scratch on the front left door" rather than "scratch, 0.81". The defect model scores 0.12 mask mAP50, and every gate, merge and abstention on the board exists because of that number.',
      image: './diagrams/arch_autoinspect.png',
      alt: 'Architecture board in three registers, drawn as a loop. Perception, top row: walk-around photos from one pass around the car enter a quality gate that rejects blur, bad framing and poor coverage; accepted frames go to two YOLO11-seg models running in parallel, one segmenting 23 body panels and one segmenting 8 defect types; their masks are intersected by intersection-over-union so each defect is placed on exactly one panel, producing a finding such as a scratch on the front left door rather than a bare class and a confidence score; repeat views are then merged so the same dent photographed from three angles counts once instead of tripling the estimate. A code artifact shows the intersection: for each defect mask the panel with the highest IoU is chosen, and a defect that cannot be placed above a minimum overlap abstains rather than becoming a line item. Decision and pricing, second register, running back from right to left: an assignment gate sends confident findings to repair operations, which are priced against an auditable, versioned rate card into a report of line items and a total, delivered live over FastAPI, WebSockets and React back to the person who took the photos; findings inside a 2 percent margin are refused and handed to a human review queue instead of being assigned on a coin flip. What the models actually score, bottom register: panel segmentation reaches 0.65 mask mAP50 across 23 classes from 3,156 training images; defect segmentation reaches 0.12 across 8 classes from 655 images, reported per class including the two of the eight that did not learn at all. The board states plainly that a 0.12 model does not get to decide, and that the quality gate, the intersection against a five-times-stronger model, the view merge, the 2 percent margin and the human queue are all consequences of that number.',
      footer: '// mask mAP50 0.65 on 23 panels, 0.12 on 8 defect types — reported per class, including the two that did not learn',
      tags: ['YOLO11-seg', 'Instance segmentation', 'PyTorch', 'Abstention', 'FastAPI', 'WebSockets'],
    },
    {
      key: 'auto-en',
      badge: 'Automation',
...
const es: Content = {
  ...
  systems: [
    ...
    },
    {
      ...
      badge: 'Visión Computacional',
      badgeType: 'vision',
      title: 'Dos Modelos, Intersectados — AutoInspect AI',
      desc: 'Una vuelta con fotos se convierte en un presupuesto de reparación. Dos modelos YOLO11-seg — 23 paneles de carrocería y 8 tipos de daño — se intersectan máscara contra máscara, así un hallazgo dice "rayón en la puerta delantera izquierda" y no "rayón, 0,81". El modelo de daños obtiene 0,12 de mask mAP50, y cada compuerta, fusión y abstención del diagrama existe por ese número.',
      image: './diagrams/arch_autoinspect.png',
      alt: 'Diagrama de arquitectura en tres registros, dibujado como un bucle. Percepción, fila superior: las fotos de una vuelta alrededor del auto entran por una compuerta de calidad que rechaza desenfoque, mal encuadre y cobertura insuficiente; los frames aceptados van a dos modelos YOLO11-seg en paralelo, uno que segmenta 23 paneles de carrocería y otro que segmenta 8 tipos de daño; sus máscaras se intersectan por intersección sobre unión de modo que cada daño queda ubicado en exactamente un panel, produciendo un hallazgo como un rayón en la puerta delantera izquierda en lugar de una clase suelta con su confianza; después las vistas repetidas se fusionan, para que el mismo golpe fotografiado desde tres ángulos cuente una vez y no triplique el presupuesto. Un artefacto de código muestra la intersección: para cada máscara de daño se elige el panel con mayor IoU, y un daño que no puede ubicarse por encima de un solapamiento mínimo se abstiene en vez de convertirse en un ítem del presupuesto. Decisión y precio, segundo registro, corriendo de derecha a izquierda: una compuerta de asignación envía los hallazgos confiables a operaciones de reparación, que se valorizan contra un tarifario auditable y versionado en un informe de ítems y total, entregado en vivo sobre FastAPI, WebSockets y React a la misma persona que tomó las fotos; los hallazgos dentro de un margen del 2 por ciento se rechazan y pasan a una cola de revisión humana en lugar de asignarse al azar. Lo que los modelos realmente puntúan, registro inferior: la segmentación de paneles alcanza 0,65 de mask mAP50 en 23 clases con 3.156 imágenes de entrenamiento; la segmentación de daños alcanza 0,12 en 8 clases con 655 imágenes, reportado por clase incluyendo las dos de las ocho que no aprendieron. El diagrama dice sin rodeos que un modelo de 0,12 no decide, y que la compuerta de calidad, la intersección contra un modelo cinco veces más fuerte, la fusión de vistas, el margen del 2 por ciento y la cola humana son todos consecuencia de ese número.',
      footer: '// mask mAP50 0,65 en 23 paneles, 0,12 en 8 tipos de daño — reportado por clase, incluidas las dos que no aprendieron',
      tags: ['YOLO11-seg', 'Segmentación de instancias', 'PyTorch', 'Abstención', 'FastAPI', 'WebSockets'],
    },
    {
      key: 'auto-es',
...
const pt: Content = {
  ...
  systems: [
    ...
    },
    {
      ...
      badge: 'Visão Computacional',
      badgeType: 'vision',
      title: 'Dois Modelos, Interseccionados — AutoInspect AI',
      desc: 'Uma volta com fotos vira um orçamento de reparo. Dois modelos YOLO11-seg — 23 painéis de carroceria e 8 tipos de avaria — são interseccionados máscara contra máscara, então um achado diz "risco na porta dianteira esquerda" e não "risco, 0,81". O modelo de avarias faz 0,12 de mask mAP50, e cada portão, fusão e abstenção do diagrama existe por causa desse número.',
      image: './diagrams/arch_autoinspect.png',
      alt: 'Quadro de arquitetura em três registros, desenhado como um laço. Percepção, linha de cima: as fotos de uma volta em torno do carro entram por um portão de qualidade que rejeita borrão, enquadramento ruim e cobertura insuficiente; os frames aceitos vão para dois modelos YOLO11-seg em paralelo, um segmentando 23 painéis de carroceria e outro segmentando 8 tipos de avaria; as máscaras dos dois são interseccionadas por interseção sobre união, de modo que cada avaria fica colocada em exatamente um painel, produzindo um achado como um risco na porta dianteira esquerda em vez de uma classe solta com sua confiança; depois as vistas repetidas são fundidas, para que o mesmo amassado fotografado de três ângulos conte uma vez e não triplique o orçamento. Um artefato de código mostra a interseção: para cada máscara de avaria escolhe-se o painel de maior IoU, e uma avaria que não pode ser colocada acima de uma sobreposição mínima se abstém em vez de virar item de orçamento. Decisão e precificação, segundo registro, correndo da direita para a esquerda: um portão de atribuição manda os achados confiáveis para operações de reparo, que são precificadas contra uma tabela auditável e versionada num laudo de itens e total, entregue ao vivo sobre FastAPI, WebSockets e React de volta para a mesma pessoa que tirou as fotos; achados dentro de uma margem de 2 por cento são recusados e vão para uma fila de revisão humana em vez de serem atribuídos no cara ou coroa. O que os modelos de fato pontuam, registro de baixo: a segmentação de painéis atinge 0,65 de mask mAP50 em 23 classes com 3.156 imagens de treino; a segmentação de avarias atinge 0,12 em 8 classes com 655 imagens, reportado por classe incluindo as duas das oito que não aprenderam. O quadro diz sem rodeios que um modelo de 0,12 não decide, e que o portão de qualidade, a interseção contra um modelo cinco vezes mais forte, a fusão de vistas, a margem de 2 por cento e a fila humana são todos consequência desse número.',
      footer: '// mask mAP50 0,65 em 23 painéis, 0,12 em 8 tipos de avaria — reportado por classe, incluídas as duas que não aprenderam',
      tags: ['YOLO11-seg', 'Segmentação de instâncias', 'PyTorch', 'Abstenção', 'FastAPI', 'WebSockets'],
    },
    {
      key: 'auto-pt',
      badge: 'Automação',
